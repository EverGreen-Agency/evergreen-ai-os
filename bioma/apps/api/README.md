...
`projects` segue router → service → repository e exige `view` para leitura e `manage_work` para escrita. O planejador cria versões a partir de contrato, briefing ou onboarding; aprovação usa capability `approve`, recusa `client_user` para o checkpoint operacional e materialização usa `manage_work`, lock e mapeamento por item para replay sem duplicar entregas. Documentos Tech podem receber `contract_id` e `planning_excerpt`; o serviço valida a pertença ao projeto e fornece ao planejador somente documentos gerais ou daquele contrato. URL não dispara busca nem leitura externa. O subdomínio Tech adiciona fases, documentos, atualizações e candidatos GitHub; Growth e Social usam o mesmo plano sem escrita GitHub. `smoke_projects.py` cobre papéis, BOLA cliente A→B, owner, escopo/fase cruzados, contrato, entrega, ritmo, feed Tech e auditoria.
