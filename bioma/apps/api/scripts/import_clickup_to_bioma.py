...
def run_import():
    print("🚀 Iniciando migração COMPLETA de todos os Espaços, Pastas e Templates do ClickUp...")

    with httpx.Client(timeout=30) as client:
        r = client.get(f"https://api.clickup.com/api/v2/team/{team_id}/space", headers=headers)
        if r.status_code != 200:
            print(f"❌ Erro ao acessar ClickUp API: {r.status_code} - {r.text}")
            return
        ...
        with connect() as conn:
            with conn.cursor() as cur:
                ...
                for space in spaces:
                    ...
                    space_id = space["id"]
                    print(f"\n🌐 Processando Espaço ClickUp: '{space_name}' (ID: {space_id})...")
                    ...
                    for folder in folders:
                        ...
                        kind = "client"
                        if "TEMPLATE" in folder_name.upper():
                            kind = "client" # criar como workspace de cliente modelo
                        elif space_name in ["Estratosfera", "EverGreen | Gestão"]:
                            kind = "client" # permitir workspaces para todas as pastas operacionais

                        # Buscar ou criar cliente/workspace no Bioma
                        cur.execute("""
                        ...
                        if not matched:
                            print(f"\n✨ Criando novo Cliente e Workspace no Bioma: '{target_name}'...")
                            cur.execute("""
                            ...
                            cur.execute("""
                                ...
                                RETURNING id
                            """, (client_org_id, target_name, folder_id))
                            ...
                            cur.execute("""
                                ...
                                RETURNING id
                            """, (eg_org_id, client_org_id, target_name, slugify(target_name)))
                            workspace_id = str(cur.fetchone()["id"])

                            # Atribuir permissões EG Admin
                            ...
                            print(f"\n📦 Atualizando Workspace existente: '{matched['client_name']}'...")

                        # Importar listas da pasta
                        l_r = client.get(f"https://api.clickup.com/api/v2/folder/{folder_id}/list", headers=headers)
                        lists = l_r.json().get("lists", [])
                        
                        _import_lists(client, cur, workspace_id, lists)

                conn.commit()
                print("\n🎉 MIGRAÇÃO COMPLETA DE TODOS OS ESPAÇOS E TEMPLATES CONCLUÍDA!")

def _import_lists(client: httpx.Client, cur, workspace_id: str, lists: list[dict]):
    for l in lists:
        list_name = l["name"]
        list_id = l["id"]
        ...
        print(f"   ✅ Lista '{list_name}': {imported_count} tarefas processadas.")
