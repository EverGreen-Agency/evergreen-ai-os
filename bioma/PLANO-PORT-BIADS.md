...
| Edge Function Deno `run-sync` | Worker Python (`apps/worker/`) com Redis (RQ ou arq); usar lib `google-auth` (assertion flow nativo — elimina o JWT manual) |
| `pg_cron` 2x/dia | Scheduler do worker (rq-scheduler/APScheduler) ou cron do Railway |
