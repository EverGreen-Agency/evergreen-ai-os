...
O domínio `projects` materializa `workspace → project → project_contracts → contract_scope_items → project_plans → project_plan_items → project_phases → deliverables`, com `project_documents` e `project_updates` como registros de acompanhamento. Um documento pode apontar para um contrato específico e conservar um `planning_excerpt` confirmado; o serviço valida que o contrato pertence ao mesmo projeto e inclui apenas documentos gerais ou daquele contrato no snapshot do planejador. URL é referência navegável, não evidência de leitura automática de conteúdo privado. Contrato e plano são versionados; o plano permanece rascunho até aprovação e sua materialização é idempotente. Itens preservam o vínculo com escopo e separam visibilidade interna/cliente. Tech, Growth e Social usam o mesmo motor, mas somente Tech marca tarefas elegíveis ao adapter GitHub. Conclusão e aceite continuam sinais distintos. O banco valida coerência de projeto, contrato, escopo, plano e entrega por FKs e triggers.
